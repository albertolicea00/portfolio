...
from html_i18n_baseline import build_baseline_from_html, DYNAMIC_ONLY_KEYS
...
def compare_keys(base_dict, target_dict, path=""):
    ...
    A handful of keys only ever exist as fallback string literals in
    script.js (dynamic/transient UI states with no static HTML form — see
    DYNAMIC_ONLY_KEYS in html_i18n_baseline.py) so the HTML-derived baseline
    can't have them; locale files are allowed to carry them without being
    flagged as "extra".
    """
    errors = []
    ...
    for k in target_dict:
        current_path = f"{path}.{k}" if path else k
        if k not in base_dict and current_path not in DYNAMIC_ONLY_KEYS:
...
def main():
    ...
    for f_name in json_files:
        ...
        # 1. Compare recursive dictionary keys (against "home" only —
        # "projects"/"experience" are lists, checked in detail below)
        errors = compare_keys(en_data["home"], data.get("home", {}))
        if errors:
